...
<template>
	<div class="flex grow flex-col gap-4">
		<div class="flex flex-col items-end gap-4">
			<button class="btn btn-error btn-sm self-start" @click="resetChatSettings">Reset to default</button>
			<div class="form-control mb-4 w-full">
				<p class="mb-1 text-sm font-medium text-primary">Prompt prefix</p>
				<textarea
					v-model="tempSettings.prefix"
			...
			<template v-for="(v, k) in tempSettings" :key="k">
				<div v-if="typeof v === 'boolean'" class="flex gap-2">
					<p>
						{{
							join(
								map(split(k.toString(), '_'), s => capitalize(s)),
								' ',
							)
						}}
					</p>
		...
		<button class="btn btn-success btn-sm mt-auto normal-case" @click="saveChatSettings">Save</button>
	</div>
</template>
